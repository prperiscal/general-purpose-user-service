package com.general.purpose.user.oauth.converter;
...
import com.general.purpose.user.oauth.model.UserDetails;
...
public class UserAuthenticationConverter implements org.springframework.security.oauth2.provider.token.UserAuthenticationConverter {
    ...
    private static UserDetails extractUserDetails(Map<String, Object> userDetailsMap) {
        return UserDetails.builder().id(UUID.fromString(userDetailsMap.get(PROP_ID).toString())).tenantId(
                UUID.fromString(userDetailsMap.get(PROP_TENANT_ID).toString())).name(
                userDetailsMap.get(PROP_NAME).toString()).password("").email(userDetailsMap.get(PROP_EMAIL).toString())
